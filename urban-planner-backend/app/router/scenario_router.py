...
from app.services.hotspot import _minmax  # dùng lại
import numpy as np
...
class ScenarioBody(BaseModel):
    ...
    month: str | None = Field(None, description="YYYY-MM; nếu None áp cho tháng mới nhất")
    NDVI: str | None = Field(None, description="+10% hoặc -5%")
...
def scenario(body: ScenarioBody):
    ...
    if month not in set(df["month"]):
        raise HTTPException(404, f"No data for {body.district} at month {month}")
    ...
    # tính risk trước & sau (min-max theo tháng)
    def risk(df):
        tmp = df.copy()
        ...
        tmp["risk"] = (1 - tmp["NDVI_norm"]) * 0.35 + tmp["AOD_norm"] * 0.30 + tmp["LST_norm"] * 0.25 + tmp["POP_norm"] * 0.10
        return tmp
